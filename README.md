...
The key idea behind the system is:

> **AI recommends. Deterministic policy controls.**

The AI can recommend an action, but it does not directly control the recovery workflow. A deterministic policy layer validates the recommendation before it can be approved.
...
Some failures may be temporary and worth retrying. Others may require customer action, human review, or should simply be stopped.

Repeatedly retrying every failed payment can waste recovery effort, while stopping too early can leave recoverable revenue behind.
...
The AI produces a structured recommendation containing information such as:
...
FOLLOW_UP_NOW
FOLLOW_UP_LATER
PAYMENT_LINK / RECOVERY_PATH
HUMAN_ESCALATION
STOP

The AI is used for decision intelligence, rather than unrestricted autonomous execution.

2. Deterministic Policy Guard

The policy layer validates AI recommendations using deterministic rules.
...
AI decides what could be done. Policy decides what is allowed.
...
The classifier uses information available from payment errors, including:
...
RIDE calculates revenue risk at the order/case level, rather than adding the amount of every payment attempt.
...
This prevents repeated attempts from artificially increasing the amount of revenue at risk.

5. Recovery Case Management

RIDE treats payment attempts and revenue cases as different concepts.

A single order may contain several payment attempts:
...
The case view also respects the selected payment attempt so that later events do not incorrectly appear in an earlier case snapshot.

6. Razorpay Test Mode Integration

RIDE uses Razorpay Test Mode to demonstrate the payment lifecycle without using real customer money.
...
Relevant payment events include:
...
Test Mode is used for development and demonstration. No production merchant or customer payment data is required.

7. Recovery Decisions & Audit Trail

Recovery decisions are stored so that the reasoning behind a decision can be reviewed later.
...
The evaluation data includes fields such as:
...
This distinction is maintained so that test payment events are not presented as real production outcomes.
...
High-level recovery and payment intelligence, including revenue at risk and case activity.

Recovery Cases

Detailed view of payment cases, attempts, failures, recovery decisions, and timelines.

AI Decisions

AI recommendations, confidence, reasoning, and final decisions.

Audit Trail

Historical recovery decision information for traceability.

Evaluation

AI versus deterministic baseline evaluation results.

Test Payment

A controlled interface for creating Razorpay Test Mode orders and testing payment flows.
